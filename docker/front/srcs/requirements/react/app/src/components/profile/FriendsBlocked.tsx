...
export default function FriendsBlocked({isBlocked, setIsBlocked} : any ){
    ...
    useEffect(() => {
        fetch(serverUrl + "/user/blocks/", {
          ...
          .then((data) => {
            ...
            console.log("Number of Blocked user:", data.users.length, nbUserBlocked);
          })
  ...
  return (
    ...
    <div className="user-blocked-div">
      <img src={blockedList && serverUrl + blockedList.users[0].avatar} alt='avatar of a user blocked' className="user-blocked-avatar"/>
      <p className="user-blocked-p">{blockedList && blockedList.users[0].name}</p>
      <button className="user-unblocked-button"   
      type="submit"
      onClick={(e: any) => {
      e.preventDefault();
      setIsBlocked(false);
      BlockAUser(blockedList.id);
      }}>Unblock</button>
    </div>
    <div className="user-blocked-div">
      <img src={avatar} alt='avatar of a user blocked'  className="user-blocked-avatar"/>
